/* eslint-disable no-case-declarations,no-constant-condition,eqeqeq,@typescript-eslint/no-non-null-assertion */
/**
 * @license
...
import { DEFAULT_INTERPOLATION_CONFIG, type InterpolationConfig } from '../ml_parser/interpolation_config'
import {
    TokenType as MlParserTokenType,
    type InterpolatedAttributeToken,
...
import { EOF, TokenType, type Lexer, type Token } from './lexer'

export type InterpolationPiece = {
    text: string
    start: number
    end: number
}
...
export class Parser {
    ...
    splitInterpolation(
        ...
        const { start: interpStart, end: interpEnd } = interpolationConfig
    ...
    private _parseBindingAst(
        ...
        const sourceToLex = this._stripComments(input)
        const tokens = this._lexer.tokenize(sourceToLex)
        return new _ParseAST(input, location, absoluteOffset, tokens, ParseFlags.None, this.errors, 0).parseChain()
    }
    ...
    private _checkNoInterpolation(input: string, location: string, { start, end }: InterpolationConfig): void {
        let startIndex = -1
        let endIndex = -1
    ...
    /**
     * Finds the index.ts of the end of an interpolation expression
...
export class _ParseAST {
    ...
    // A serial of the expression start and input index.ts is used for mapping because both are stateful
    ...
    /**
     * End index.ts of the last processed token, or the start of the first token if none have been
     * processed.
     */
     ...
     * @param artificialEndIndex Optional ending index.ts to be used if provided (and if greater than the
     *     natural ending index.ts)
     */
        ...
        if (!this.consumeOptionalCharacter(chars.$RBRACE)) {
            ...
            do {
                ...
                if (quoted) {
                    ...
                    values.push(
                        new PropertyRead(span, sourceSpan, sourceSpan, new ImplicitReceiver(span, sourceSpan), key),
                    )
                }
            } while (this.consumeOptionalCharacter(chars.$COMMA) && !this.next.isCharacter(chars.$RBRACE))
            this.rbracesExpected--
     ...
     * For example,
     * ```
     *   <div *ngFor="let item of items; index.ts as i; trackBy: func">
     * ```
     * contains five bindings:
     * 1. ngFor -> null
     * 2. item -> NgForOfContext.$implicit
     * 3. ngForOf -> items
     * 4. i -> NgForOfContext.index.ts
     * 5. ngForTrackBy -> func
     *
    ...
    private consumeOptionalAssignment(): boolean {
        ...
        if (this.parseFlags & ParseFlags.AssignmentEvent && this.next.isOperator('!') && this.peek(1).isOperator('=')) {
     ...
     * ```
     *   *ngFor="let item of items; index.ts as i"
     *                              ^^^^^    ^
     *                              value    key
     * ```
     *
     ...
     * ```
     *   *ngFor="let item of items; let i=index.ts;"
     *           ^^^^^^^^           ^^^^^^^^^^^
     * ```
     * In the first binding, `item` is bound to `NgForOfContext.$implicit`.
     * In the second binding, `i` is bound to `NgForOfContext.index.ts`.
     */
 ...
 *
 * @returns A map of index.ts locations in the decoded template to indexes in the original template
 */
