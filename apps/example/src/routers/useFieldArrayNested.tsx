import { Control, Controller, useFieldArray, useForm, useFormState } from '@worldprinter/formeasy'
...
function NestedArray({ control, index }: { control: Control<FormValues>; index: number }) {
    const { fields, append, prepend, swap, move, remove, insert, update } = useFieldArray<
        FormValues,
        'test.0.keyValue'
    >({
...
export default () => {
    const { register, control, reset, setValue, handleSubmit } = useForm<FormValues>({
        defaultValues: {
            test: [
                {
                    firstName: 'Bill',
    ...
    const { fields, append, prepend, swap, move, insert, remove, update } = useFieldArray({
        control,
        name: 'test',
    })
    const renderCountRef = React.useRef(0)
    ...
    return (
        <form onSubmit={handleSubmit((e) => setResult(e))}>
            ...
            <button
                ...
                onClick={() =>
                    update(0, {
                        ...
                        keyValue: [{ name: 'updateFirstName1' }, { name: 'updateFirstName2' }],
                    })
                }
                type='button'
            >
            ...
            <button
                ...
                onClick={() => {
                    reset({
                        test: [{ firstName: 'test' }, { firstName: 'test1' }, { firstName: 'test2' }],
                    })
                }}
            >
                reset
