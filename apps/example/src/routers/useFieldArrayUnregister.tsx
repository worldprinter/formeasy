import { Control, Controller, useFieldArray, useForm, UseFormReturn, useWatch } from '@worldprinter/formeasy'
...
const ConditionField = <T extends any[]>({
    ...
    return output?.[index]?.name === 'bill' ? <input {...control.register(`data.${index}.conditional`)} /> : null
}
    ...
    return (
        ...
        >
            <ul>
                {fields.map((data, index) => (
                    <li key={data.id}>
                        ...
                        {errors.data?.[index]?.name && <p id={`error${index}`}>{errors.data[index]!.name!.message}</p>}
            ...
            <button
                id='append'
                type='button'
                onClick={() => append({ name: renderCount.toString(), conditional: '' })}
            >
                append
            </button>
            ...
            <button
                id='prepend'
                type='button'
                onClick={() => prepend({ name: renderCount.toString(), conditional: '' })}
            >
                prepend
            </button>
            ...
            <button
                id='insert'
                type='button'
                onClick={() => insert(1, { name: renderCount.toString(), conditional: '' })}
            >
                insert
            </button>
